...
class JobController extends Controller
{
    ...
    function getAllForUser(Request $request, $user_id){
        ...
        $jobs_auth = Job::where("user_id", $user_id)->first();
        if($jobs_auth!=null){
            return response()->json([
                "resp"=>$jobs
            ]);
    ...
    function getAllForWorker(Request $request, $user_id){
        $jobs = Job::where("worker_id", $user_id)->get();
        $jobs_auth = Job::where("worker_id", $user_id)->first();
        if($jobs_auth!=null){
            return response()->json([
                "resp"=>$jobs
            ]);
